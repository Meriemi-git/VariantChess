...
public class KingMoveSet extends AbstractMoveSet implements GameEventSubscriber {
    ...
    protected fr.aboucorp.variantchess.entities.utils.SquareList getPossibleMoves(Piece piece, ChessColor turnColor) {
        fr.aboucorp.variantchess.entities.utils.SquareList validSquares = getClassicMoves(piece,turnColor);
        if(piece.getChessColor() == turnColor) {
            if (turnColor == ChessColor.WHITE) {
                ...
                this.eventManager.sendMessage(new fr.aboucorp.variantchess.entities.events.models.LogEvent(String.format("GetPossibleMoves %s => canCastleQueenSide : %s ; canCastleKingSide : %s",this.actualTurn.getTurnColor(),canCastleQueenSide,canCastleKingSide)));
    ...
    @Override
    public fr.aboucorp.variantchess.entities.utils.SquareList getThreats(Piece piece, ChessColor turnColor) {
    ...
    public void receiveGameEvent(GameEvent event) {
        ...
        if(event instanceof fr.aboucorp.variantchess.entities.events.models.PieceEvent && this.piece.getChessColor() == ((fr.aboucorp.variantchess.entities.events.models.PieceEvent) event).piece.getChessColor()){
            this.canCastleQueenSide = ((fr.aboucorp.variantchess.entities.events.models.PieceEvent) event).type == BoardEventType.CASTLE_QUEEN;
            this.canCastleKingSide = ((fr.aboucorp.variantchess.entities.events.models.PieceEvent) event).type == fr.aboucorp.variantchess.entities.enums.BoardEventType.CASTLE_KING;
    ...
    public fr.aboucorp.variantchess.entities.utils.SquareList getClassicMoves(Piece piece, ChessColor turnColor){
        fr.aboucorp.variantchess.entities.utils.SquareList allSquares = classicBoard.getSquares();
        fr.aboucorp.variantchess.entities.utils.SquareList validSquares = new SquareList();
        fr.aboucorp.variantchess.entities.Location start = piece.getLocation();
        Square up = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()+1,0,start.getZ()));
        if(up != null
                && (up.getPiece() == null || up.getPiece().getChessColor() != turnColor)
                && (turnColor != piece.getChessColor() || isLocationSafe(up,turnColor))){
            validSquares.add(up);
        }
        Square upRight = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()+1,0,start.getZ()-1));
        if(upRight != null
        ...
        Square upLeft = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()+1,0,start.getZ()+1));
        if(upLeft != null
        ...
        Square down = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()-1,0,start.getZ()));
        if(down != null
        ...
        Square downLeft = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()-1,0,start.getZ()+1));
        if(downLeft != null
        ...
        Square downRight = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX()-1,0,start.getZ()-1));
        if(downRight != null
        ...
        Square left = (Square) allSquares.getItemByLocation(new fr.aboucorp.variantchess.entities.Location(start.getX(),0,start.getZ()+1));
        if(left != null
